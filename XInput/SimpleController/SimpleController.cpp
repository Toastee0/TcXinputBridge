...
#include "c:\twincat\adsapi\tcadsdll\include\TcAdsDef.h"
#include "c:\twincat\adsapi\tcadsdll\include\TcAdsAPI.h"
...
//added for beckhoff
char      szVar[] = { "JSDATA.GamepadState" };
...
bool    g_bDeadZoneOn = true;
  //added beckhoff stuff
    long     nErr, nPort;
    AmsAddr  Addr;
    PAmsAddr pAddr = &Addr;
    ULONG     lHdlVar;
    AmsNetId myNetId = { 10,148,20,235,1,1 }; //10.147.20.235.1.1
    ...
    while( WM_QUIT != msg.message )
    {
        ...
        else
        {
            ...
            UpdateControllerState();
             memcpy_s(&pData[0], 16, &g_Controllers[0].state, 16);
...
void RenderFrame()
{
    ...
    for( DWORD i = 0; i < MAX_CONTROLLERS; i++ )
    {
        if( g_Controllers[i].bConnected )
        {
            ...
            // send controller state here

            if (g_Controllers[0].state.dwPacketNumber == lastdwPacketNumber) {
                lastdwPacketNumber = g_Controllers[0].state.dwPacketNumber;
                swprintf_s(sz[i], 1024, L"packet num %d", g_Controllers[0].state.dwPacketNumber);
            }
            else
            {
                lastdwPacketNumber = g_Controllers[0].state.dwPacketNumber;
        ...
        {
            //send controller error instead here
